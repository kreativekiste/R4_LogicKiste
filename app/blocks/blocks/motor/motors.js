...
// Hilfsfunktion: Servo einmalig pro Pin registrieren
function _initServo(pin) {
    ...
    // KEIN usedPinsOutput.add() — Servo braucht kein pinMode(OUTPUT)!
    // Der Core würde sonst pinMode(pin9, OUTPUT) vor attach() schreiben,
    // was auf dem R4 den Servo-Betrieb stört.

    // Pin-Konstante manuell anlegen (ohne Core-Registrierung)
    ArduinoGenerator.globals_.add(`const int pin${pin} = ${pin};`);

    // Servo-Instanz global
