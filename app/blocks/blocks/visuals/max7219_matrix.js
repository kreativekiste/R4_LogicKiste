...
ArduinoGenerator.hardwareScanners['max7219_setup'] = function(block) {
    ...
    // Autarker Setup-Wächter (kugelsicher)
    if (!ArduinoGenerator.includes_.has('#include <MD_Parola.h>')) {

        // Bibliotheken registrieren
        ...
        // #define verwenden, um Speicher zu sparen
        ArduinoGenerator.globals_.add(`#define MAX7219_CS ${cs}`);
        ArduinoGenerator.globals_.add(`#define MAX7219_NUM ${num}`);

        // Hardware-Objekt global anlegen
...
ArduinoGenerator.forBlock['max7219_animation'] = function(block) {
    ...
    // Parola verlangt zwingend C-Strings (char array) für Laufschriften, keine Arduino Strings.
    // Daher wandeln wir den Text sicher in ein char-Array um und füttern dann die Animations-Engine.
    let code = `  // --- Matrix Animation ---\n`;
    code += `  String t_str = String(${text});\n`;
    code += `  static char t_char[60];\n`;
    code += `  t_str.toCharArray(t_char, 60);\n`;
    code += `  if (P.displayAnimate()) {\n`;
    code += `    P.displayText(t_char, ${align}, ${speed}, ${pause}, ${effIn}, ${effOut});\n`;
    code += `    P.displayReset();\n`;
    code += `  }\n`;
